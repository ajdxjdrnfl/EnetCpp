...
{               
    ...
    incomingCommand = reinterpret_cast<ENetIncomingCommand*>(peer->dispatchedCommands.remove(peer->dispatchedCommands.begin()));
...
static void enet_peer_remove_incoming_commands(ENetPeer* peer, ENetList* queue, ENetList::iterator startCommand, ENetList::iterator endCommand, ENetIncomingCommand* excludeCommand)
{
...
{
    enet_peer_remove_incoming_commands(peer, queue, queue->begin(), queue->end(), nullptr);
}
...
void enet_peer_reset_queues(ENetPeer* peer)
{
    ...
    while (!peer->acknowledgements.empty())
        enet_free(peer->acknowledgements.remove(peer->acknowledgements.begin()));
