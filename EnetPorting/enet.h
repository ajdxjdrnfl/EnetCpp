...
{
    ENetListNode outgoingCommandList; // 현재 Commandm각 속한 List의 역참조 같은 느낌
...
struct ENetChannel
{
    ...
    enet_uint16  incomingReliableSequenceNumber; // 채널에서 마지막으로 확인한 SeqNumber
...
{
public:
    ENetSocket(SOCKET socket = ENET_SOCKET_NULL) : socket(socket) { }
...
public:
    ...
    std::array<ENetPeer*, ENET_PROTOCOL_MAXIMUM_PEER_ID> peers;                       /**< array of peers allocated for this host */
