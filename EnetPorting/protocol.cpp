...
static void enet_protocol_dispatch_state(ENetHost* host, ENetPeer* peer, ENetPeerState state)
{
    ...
    {
        host->dispatchQueue.insert(host->dispatchQueue.end(), &peer->dispatchList);
...
void enet_protocol_remove_sent_unreliable_commands(ENetPeer* peer, ENetList* sentUnreliableCommands)
{
    ...
    if (sentUnreliableCommands->empty())
        return;
    ...
    } while (!sentUnreliableCommands->empty());
...
// OnSendReliable - 여기서 Send는 peer에게 보낸게 아니라, peer가 보낸거인듯. 사실상 OnReceive의 역할,,
...
static int enet_protocol_send_outgoing_commands(ENetHost* host, ENetEvent* event, int checkForTimeouts)
{
    ...
    {
        int i = 0;
        for (ENetPeer* currentPeer = host->peers[i];
            i < host->peers.size() && currentPeer;
            currentPeer = host->peers[i], i++)
        {
            if (currentPeer->state == ENET_PEER_STATE_DISCONNECTED ||
